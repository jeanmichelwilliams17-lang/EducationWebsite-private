...
def find_backup_profile(primary: str = "") -> Optional[str]:
    disabled = _load_disabled()
    try:
        from notebooklm.paths import list_profiles
        for name in list_profiles():
            if name == primary or name in disabled:
                continue
            if is_rate_limited(name) is None:
...
async def _submit_async(
    ...
    resolved_prompt = prompt  # Variable substitution is handled by the caller

    async with NotebookLMClient.from_storage(profile=profile, chat_timeout=600) as client:
        nb = await client.notebooks.create(f"Chatter — {output_label}")
        notebook_id = nb.id
        answer_text = ""
        try:
            if source_path:
                ...
                for idx, up_path in enumerate(source_paths):
                    for attempt in range(3):
                        try:
                            await client.sources.add_file(notebook_id, up_path, wait=True)
                            break
            ...
            response = await client.chat.ask(notebook_id, resolved_prompt)
...
def submit(
    ...
    try:
        ...
        err_str = str(e)
        backup = _find_backup_profile(profile)
        if backup and not _retried_auth:
            log.warning("Auth expired for '%s'. Retrying with backup '%s'...", profile, backup)
        ...
        if _is_rate_limit(err_str):
            ...
            backup = _find_backup_profile(profile)
            if backup:
                log.info("Rate limit hit, retrying with backup %s", backup)
                try:
                    success, text, err = asyncio.run(
